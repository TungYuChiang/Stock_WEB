...
#==============================================================================#
    ...
    def changepoint_prior_analysis(self, changepoint_priors=[0.001, 0.05, 0.1, 0.2], colors=['b', 'r', 'grey', 'gold']):
        ...
        ##將圖片存在static/img   (檔名為傳入的)
        route = "static/img/"
        fig_name = route + "long_short_" + self.name + ".png"
        plt.savefig(fig_name)
        print("save figure sucess, save as " + fig_name)
        #plt.show()
    ...
    def create_prophet_model(self, days=0, resample=False):
        ...
        ##將圖片存在static/img   (檔名為傳入的)
        route = "static/img/"
        fig_name = route + "7day_pre_" + self.name + ".png"
        plt.savefig(fig_name)
        print("save figure sucess, save as " + fig_name)
        #plt.show()
    ...
    def evaluate_prediction(self, start_date=None, end_date=None, nshares = None):
        ...
        if not nshares:
            ...
            plt.title('{} Model Evaluation from {} to {}.'.format(self.name,
                start_date, end_date));

            ##將圖片存在static/img   (檔名為傳入的)
            route = "static/img/"
            fig_name = route + self.name + ".png"
            plt.savefig(fig_name)
            #plt.show();
            print("save figure sucess, save as " + fig_name)
